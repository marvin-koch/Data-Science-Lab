    ...
    if args.reference_lora_model_path:
        ...
        try:
            ...
            lora_state_dict, network_alphas = StableDiffusionXLLoraLoaderMixin.lora_state_dict(
                args.reference_lora_model_path, weight_name="LogoRedmondV2-Logo-LogoRedmAF.safetensors",  unet_config=unet.config        #  ←  add this line
            )
                
            if not (args.resume_from_checkpoint is None):
                lora_state_dict_train, network_alphas_train = StableDiffusionXLLoraLoaderMixin.lora_state_dict(
                        f"diffusion-sdxl-dpo/{args.resume_from_checkpoint}", weight_name="pytorch_lora_weights.safetensors",  unet_config=unet.config        #  ←  add this line
                    )
                
                logger.info("Loaded LORA from checkpoint!!!")
            else:
                lora_state_dict_train, network_alphas_train = lora_state_dict, network_alphas
            ...
            StableDiffusionXLLoraLoaderMixin.load_lora_into_unet(
                lora_state_dict_train, network_alphas_train,
                unet,
                adapter_name=TRAINING_ADAPTER,   # weights go into the slot above
            )

    
            # unet.add_adapter(LoraConfig(r=8, lora_alpha=8), adapter_name=REF_ADAPTER)
            StableDiffusionXLLoraLoaderMixin.load_lora_into_unet(
            ...
            # unet.load_lora_weights(args.reference_lora_model_path, adapter_name=TRAINING_ADAPTER)      # trainable copy
            # unet.load_lora_weights(args.reference_lora_model_path, adapter_name=REFERENCE_ADAPTER, freeze=True)
            ...
            # for p in unet.get_adapter(TRAINING_ADAPTER).parameters():
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            ...
            logs = {
                ...
                "inside_term_mean": inside_term.mean().item(),
