...
# from validation_step_rlhf import run_order # Assuming this is a custom module you have
...
# Placeholder for run_order if not available
def run_order(image_dir, base_save_path, prompts_short, step):
    logger.warning(f"Function 'run_order' from 'validation_step_rlhf' is not available. Skipping metric calculation for step {step}.")
    return {"custom_validation_metric": 0.0}
...
def log_validation(args, unet, vae, accelerator, weight_dtype, epoch, global_step, is_final_validation=False):
    ...
    if not is_final_validation:
        ...
        # Ensure the LoRA weights are loaded from the correct final output directory
        lora_weights_path = os.path.join(args.output_dir, "pytorch_lora_weights.safetensors")
        if os.path.exists(lora_weights_path):
            logger.info(f"Loading LoRA weights from {lora_weights_path} for final validation.")
            pipeline.load_lora_weights(args.output_dir, weight_name="pytorch_lora_weights.safetensors")
        else:
            logger.warning(f"LoRA weights not found at {lora_weights_path}. Final validation might not use trained LoRA.")
    ...
    logger.info(f"Saving validation images to {step_save_dir}")
    ...
    for batch_num in range(num_batches):
        ...
        with context:
            logger.debug(f"Generating validation batch {batch_num+1}/{num_batches}...")
            results = pipeline(
                prompt_batch,
    ...
    try:
        ...
        logger.warning("validation_step_rlhf.run_order not found. Skipping custom metrics.")
    ...
    for tracker in accelerator.trackers:
        ...
        if tracker.name == "wandb":
            try:
                ...
                logger.warning(f"Failed to log images to WandB: {e}")
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--dataset_split_name",
        type=str,
        default="train", # Changed default to 'train' as 'validation' is unusual for main training
    ...
    parser.add_argument(
        "--beta_dpo",
        type=float, # Changed to float to allow finer control, DPO paper uses 0.1 for some tasks
        default=5000.0, # Default from original script
        help="DPO KL Divergence penalty (or beta for GDPO scaling). For GDPO, a higher beta might be needed as per paper.",
    )
    parser.add_argument(
        "--learning_rate",
        type=float,
        default=5e-4, # Common for LoRA
    ...
    parser.add_argument(
        "--use_gdpo",
        action="store_true",
        help="Whether to use Geometric DPO (GDPO) with soft labels instead of standard DPO.",
    )
...
def main(args):
    ...
    unet_lora_config = LoraConfig(
        r=args.rank, lora_alpha=args.rank, init_lora_weights="gaussian", target_modules=["to_k", "to_q", "to_v", "to_out.0"],
    )
    ...
    if args.enable_xformers_memory_efficient_attention:
        if is_xformers_available():
            ...
            if xformers_version == version.parse("0.0.16"):
                logger.warning("xFormers 0.0.16 may cause issues. Consider updating to 0.0.17+.")
    ...
    def load_model_hook(models, input_dir):
        ...
        StableDiffusionXLLoraLoaderMixin.load_lora_into_unet(lora_state_dict, network_alphas=network_alphas, unet=unet_)
    ...
    if args.use_8bit_adam:
        try:
            ...
            raise ImportError("bitsandbytes is required for 8-bit Adam.")
    ...
    train_dataset = load_dataset(args.dataset_name, cache_dir=args.cache_dir, split=args.dataset_split_name)
    
    # --- Data Preparation Logic ---
    def prepare_dpo_data(row):
        score1 = row['weighted_results_image1_preference']
        score2 = row['weighted_results_image2_preference']
        
        if args.use_gdpo:
            prob_image1_is_better = torch.sigmoid(torch.tensor(score1 - score2, dtype=torch.float32)).item()

            if prob_image1_is_better >= 0.5:
                chosen_image_data = row['image1']
                rejected_image_data = row['image2']
                p_hat_value = prob_image1_is_better
            else:
                chosen_image_data = row['image2']
                rejected_image_data = row['image1']
                p_hat_value = 1.0 - prob_image1_is_better 
            
            return {
                'chosen_image': chosen_image_data,
                'rejected_image': rejected_image_data,
                'p_hat': p_hat_value,
                'prompt': row['prompt']
            }
        else:
    ...
    # Define columns to remove based on the current dataset and GDPO mode
    current_columns = train_dataset.column_names
    columns_to_remove_after_map = ['weighted_results_image1_preference', 'weighted_results_image2_preference']

    # If not using GDPO, 'image1' and 'image2' are kept (though potentially swapped),
    # so only remove the preference scores.
    # If using GDPO, 'image1' and 'image2' are transformed into 'chosen_image' and 'rejected_image',
    # so the original 'image1' and 'image2' can also be removed after mapping.
    if args.use_gdpo:
        columns_to_remove_after_map.extend(['image1', 'image2'])
        # Also remove any other original columns that are not needed for GDPO's output
        # (e.g., 'model1', 'model2', 'detailed_results_preference' if they exist and are not used)
        # We should only remove columns that are guaranteed to be in `current_columns`
        # and are being replaced or are no longer needed.
        for col in ['model1', 'model2', 'detailed_results_preference']:
            if col in current_columns:
                columns_to_remove_after_map.append(col)
    else: # Original DPO
        # For DPO, we might have already removed image1/image2 if they were transformed or renamed
        # 'label_0' is created, 'image1'/'image2' might be kept or renamed.
        # We definitely want to remove the preference scores.
        # Let's be more careful here. The original DPO part of prepare_dpo_data
        # *outputs* 'image1' and 'image2'. So we should NOT remove them here.
        # We only remove the original preference scores.
        # Other original columns not used by DPO's output can also be listed here
        # if they exist.
        for col in ['model1', 'model2', 'detailed_results_preference']:
             if col in current_columns:
                columns_to_remove_after_map.append(col)
    ...
    # Filter columns_to_remove_after_map to only include those present in current_columns
    actual_columns_to_remove = [col for col in columns_to_remove_after_map if col in current_columns]
    ...
    if args.use_gdpo:
        logger.info("Prepared dataset for GDPO with 'chosen_image', 'rejected_image', 'p_hat', 'prompt'.")
        logger.info(f"Dataset columns after GDPO map: {train_dataset.column_names}")
    else:
        logger.info("Prepared dataset for DPO with 'image1', 'image2', 'label_0', 'prompt'.")
        logger.info(f"Dataset columns after DPO map: {train_dataset.column_names}")
    ...
    def preprocess_train(examples):
        ...
        if args.use_gdpo:
            images_w = [im.convert("RGB") for im in examples["chosen_image"]]
            images_l = [im.convert("RGB") for im in examples["rejected_image"]]
            
            # Determine original sizes from chosen images
            original_sizes_w = [(img.height, img.width) for img in images_w]
            
            # Resize rejected images to match original size of chosen ones
            images_l_resized = [
                img_l.resize(orig_size_w[::-1]) # PIL resize takes (width, height)
                for img_l, orig_size_w in zip(images_l, original_sizes_w)
            ]
            
            pixel_values_w = [to_tensor(img) for img in images_w]
            pixel_values_l = [to_tensor(img) for img in images_l_resized]

            im_tup_iterator = zip(pixel_values_w, pixel_values_l)
            original_sizes_iterator = original_sizes_w # Use chosen image's original size
        else: # Original DPO
            images1 = [im.convert("RGB") for im in examples["image1"]]
            images2 = [im.convert("RGB") for im in examples["image2"]]
            labels_0 = examples["label_0"]

            original_sizes_1 = [(img.height, img.width) for img in images1]
            images2_resized = [
                img2.resize(orig_size1[::-1])
                for img2, orig_size1 in zip(images2, original_sizes_1)
            ]

            pixel_values1 = [to_tensor(img) for img in images1]
            pixel_values2 = [to_tensor(img) for img in images2_resized]
            
            processed_im_tups = []
            processed_original_sizes = []
            for pv1, pv2, label, orig_s1 in zip(pixel_values1, pixel_values2, labels_0, original_sizes_1):
                if label == 0: # image2 was preferred, swap them to make pv1 the winner
                    processed_im_tups.append((pv2, pv1))
                else: # image1 was preferred
                    processed_im_tups.append((pv1, pv2))
                processed_original_sizes.append(orig_s1) # Original size of the (now potentially swapped) winner

            im_tup_iterator = processed_im_tups
            original_sizes_iterator = processed_original_sizes


        for i, im_tup in enumerate(im_tup_iterator):
            current_original_size = original_sizes_iterator[i]
            original_sizes_list.append(current_original_size)
            ...
            crop_top_lefts_list.append((y1, x1))
            combined_im = normalize(combined_im)
            combined_pixel_values.append(combined_im)

        examples["pixel_values"] = combined_pixel_values
        examples["original_sizes"] = original_sizes_list
        examples["crop_top_lefts"] = crop_top_lefts_list
        
        tokens_one, tokens_two = tokenize_captions([tokenizer_one, tokenizer_two], examples) # examples["prompt"]
        examples["input_ids_one"] = tokens_one
        examples["input_ids_two"] = tokens_two
        
        if args.use_gdpo:
            examples["p_hat"] = examples["p_hat"] # Pass p_hat through
        return examples
    ...
    def collate_fn(examples):
        ...
        if args.use_gdpo:
            batch["p_hats"] = torch.tensor([example["p_hat"] for example in examples], dtype=torch.float32)
        return batch
    ...
    logger.info(f"  Using GDPO: {args.use_gdpo}")
    ...
    if args.resume_from_checkpoint:
        ...
        if path is None:
            accelerator.print(f"Checkpoint '{args.resume_from_checkpoint}' not found. Starting new run.")
            ...
            if args.mixed_precision == "fp16": # Ensure LoRA params are fp32 after load
                unet_model = accelerator.unwrap_model(unet)
                for param in unet_model.parameters():
                    if param.requires_grad:
                        param.data = param.to(torch.float32)
                logger.info("Ensured trainable LoRA parameters are in FP32 after resuming.")
    else:
        initial_global_step = 0
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                feed_pixel_values = torch.cat(pixel_values.chunk(2, dim=1)) # (2*bsz, C, H, W)
                
                latents = []
                for i in range(0, feed_pixel_values.shape[0], args.vae_encode_batch_size):
                    latents.append(vae.encode(feed_pixel_values[i : i + args.vae_encode_batch_size]).latent_dist.sample())
                latents = torch.cat(latents, dim=0) # (2*bsz, latent_C, latent_H, latent_W)
                latents = latents * vae.config.scaling_factor
                if args.pretrained_vae_model_name_or_path is None: # SDXL VAE
                    latents = latents.to(weight_dtype)

                noise = torch.randn_like(latents).chunk(2)[0].repeat(2, 1, 1, 1)
                bsz = latents.shape[0] // 2
                timesteps = torch.randint(0, noise_scheduler.config.num_train_timesteps, (bsz,), device=latents.device, dtype=torch.long).repeat(2)
                if args.is_turbo:
                ...
                def compute_time_ids(original_size, crops_coords_top_left):
                    ...
                    add_time_ids = torch.tensor([add_time_ids]).to(accelerator.device, dtype=weight_dtype)
                    return add_time_ids
                ...
                model_losses_w, model_losses_l = model_losses.chunk(2) # (bsz), (bsz)
                ...
                # (model_diff - ref_diff) is proportional to h_theta_script
                # h_theta_paper = ref_diff - model_diff
                # For GDPO, h_theta_paper is scaled by (2*p_hat - 1)
                # So, (model_diff - ref_diff) should be scaled by (2*p_hat - 1)
                
                if args.use_gdpo:
                    p_hat = batch["p_hats"].to(model_diff.device).unsqueeze(-1) # (bsz, 1) for broadcasting
                    # Ensure p_hat is on the same device and reshaped if model_diff is (bsz)
                    if p_hat.dim() > 1 and model_diff.dim() == 1: # p_hat (bsz, 1), model_diff (bsz)
                         p_hat = p_hat.squeeze(-1)

                    gdpo_coeff = (2.0 * p_hat - 1.0) # (bsz)
                    inside_term = scale_term * gdpo_coeff * (model_diff - ref_diff)
                else:
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    ...
                    if args.run_validation and global_step % args.validation_steps == 0:
                        log_validation(args, unet=unet, vae=vae, accelerator=accelerator, weight_dtype=weight_dtype, epoch=epoch, global_step=global_step)
            
            logs = {"loss": loss.detach().item(), "raw_model_loss": raw_model_loss.detach().item(), 
                    "ref_loss": raw_ref_loss.detach().item(), "implicit_acc": implicit_acc.detach().item(),
                    "lr": lr_scheduler.get_last_lr()[0]}
            if args.use_gdpo:
                logs["avg_p_hat"] = p_hat.mean().item() if 'p_hat' in locals() else -1.0 # Log average p_hat if available
                logs["avg_gdpo_coeff"] = gdpo_coeff.mean().item() if 'gdpo_coeff' in locals() else -1.0
    ...
    if accelerator.is_main_process:
        ...
        unet = unet.to(torch.float32) # Ensure full precision for saving
        unet_lora_state_dict = convert_state_dict_to_diffusers(get_peft_model_state_dict(unet))
        StableDiffusionXLLoraLoaderMixin.save_lora_weights(
            save_directory=args.output_dir, unet_lora_layers=unet_lora_state_dict,
            text_encoder_lora_layers=None, text_encoder_2_lora_layers=None, # No text encoder LoRA
        )
        ...
        if args.run_validation:
            ...
            log_validation(
                args, unet=None, vae=vae, accelerator=accelerator, weight_dtype=weight_dtype, 
                epoch=epoch, global_step=global_step, is_final_validation=True
            )
        if args.push_to_hub:
            logger.info(f"Pushing model to Hub: {repo_id if 'repo_id' in locals() else args.hub_model_id}")
            upload_folder(
                repo_id=repo_id if 'repo_id' in locals() else args.hub_model_id, 
                folder_path=args.output_dir, commit_message="End of training", 
                ignore_patterns=["step_*", "epoch_*", "checkpoint-*/*"], # Ignore temp checkpoints if any
            )
