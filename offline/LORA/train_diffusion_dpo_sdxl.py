...
def main(args):
    ...
    if args.resume_from_checkpoint:
        ...
        if path is None:
            ...
            if args.mixed_precision == "fp16":
                unet_model = accelerator.unwrap_model(unet) # Get the underlying model
                for param in unet_model.parameters():
                    if param.requires_grad:
                        param.data = param.to(torch.float32)
                logger.info("Ensured trainable parameters are in FP32 after resuming from checkpoint.")
    else:
        initial_global_step = 0
